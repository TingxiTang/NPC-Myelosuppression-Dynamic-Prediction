...
- 参考原 Web 的中文治疗方案多选，并用同一冻结模型对当前所选方案进行一次情景重算。
...
上传或选定样例后，界面会从冻结编码恢复当前治疗项目，并提供中文多选框。更改选项后，只重建以下“本次方案”字段：`drug_id`、`category_id`、`is_chemo`、`is_target`、`is_immuno`、`is_rt`；基线检验、分期、既往 nadir、累计暴露和周期状态保持不变。药物名称来自历史训练实际使用的药物字典，药物—类别关系来自原 Web 的映射表，并进一步限制为当前冻结 encoder 的词表；任一字典哈希、编码覆盖或类别关系异常都会停止预测。
